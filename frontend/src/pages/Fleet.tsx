...
export default function Fleet() {
  ...
  // Keep strictly the 25 specified fleet airframes
  const fleetAirframes = useMemo(() => {
    return airframes.filter((a) => isFleetAircraft(a.registration));
  }, [airframes]);

  const handleSyncAll = async () => {
    const linkedAirframes = fleetAirframes.filter((a) => a.if_organization_aircraft_id);
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Qatar Virtual official aircraft fleet ({fleetAirframes.length} airframes)
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-5 py-2.5 rounded-full bg-brand text-white font-bold text-sm hover:bg-brand-light transition-all duration-300 disabled:opacity-50 shadow-md hover:shadow-lg flex items-center gap-2 cursor-pointer"
          >
            {syncing ? (
              <>
      ...
      {syncResult && (
        <div className="mb-6 text-sm text-green-700 bg-green-50 border border-green-200 rounded-2xl p-4 shadow-sm flex items-center justify-between animate-fade-in">
          ...
          <button onClick={() => setSyncResult(null)} className="text-green-500 hover:text-green-700 font-bold px-2 text-base cursor-pointer">
            &times;
          </button>
        </div>
      )}
        ...
        {fleetAirframes.map((a) => {
          ...
          return (
            ...
            >
              ...
              {ifEntry && (
                <div className="mt-3 p-2.5 bg-brand-pale/60 rounded-xl border border-brand-border/40">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span className="text-gray-700 font-bold text-[11px]">{ifEntry.if_registration}</span>
                    <span
                      className={`px-1.5 py-0.5 rounded-full font-bold text-[9px] ${
                        visibilityColor[ifEntry.if_visibility] || visibilityColor[0]
                      }`}
                    >
              ...
              {isLinked && !ifEntry && (
                <div className="mt-2 text-[10px] text-gray-400 italic">Linked to IF · no live data</div>
              )}
