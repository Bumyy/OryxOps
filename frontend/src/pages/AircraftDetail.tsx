...
export default function AircraftDetail() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <Link to="/fleet" className="text-sm text-[--color-brand] hover:underline mb-4 inline-block">&larr; Back to Fleet</Link>

      <div className="flex items-start justify-between flex-wrap gap-4 mb-8">
        <div>
          <h1 className="text-5xl font-bold text-[--color-brand]">{a.registration}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-2xl border border-[--color-brand-border] shadow-sm p-4">
            <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold">{s.label}</p>
            <p className="text-lg font-bold text-[--color-brand] mt-1">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {a.history && a.history.length > 0 && (
        <div className="bg-white rounded-2xl border border-[--color-brand-border] shadow-sm overflow-hidden">
          <h2 className="text-xl font-bold text-[--color-brand] p-6 pb-4">Flight History</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {a.history.map((h: any) => (
                  <tr key={h.id} className="border-t border-[--color-brand-border]">
