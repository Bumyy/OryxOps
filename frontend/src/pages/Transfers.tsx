      ...
      <div className="bg-white rounded-2xl border border-brand-border shadow-sm p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4 items-end">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="transfer-type-select" className="text-xs font-bold text-gray-500">TRANSFER TYPE</label>
            <select id="transfer-type-select" value={type} onChange={(e) => setType(e.target.value)} className="select select-bordered select-md w-full">
              <option value="group_switch">Group Switch</option>
              <option value="career_path_switch">Career Path Switch</option>
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            <label htmlFor="transfer-to-input" className="text-xs font-bold text-gray-500">TO (Group Name / Path Name)</label>
            <input id="transfer-to-input" placeholder="e.g. Airbus Path" value={toValue} onChange={(e) => setToValue(e.target.value)} className="input input-bordered input-md w-full" />
          </div>
          <div className="flex flex-col gap-1.5">
            <label htmlFor="transfer-reason-input" className="text-xs font-bold text-gray-500">REASON (Optional)</label>
            <input id="transfer-reason-input" placeholder="Reason for request" value={reason} onChange={(e) => setReason(e.target.value)} className="input input-bordered input-md w-full" />
          </div>
        </div>
        <button onClick={handleCreate} className="btn btn-primary font-semibold text-sm px-5 transition-all">
          Submit Request
        </button>
      </div>
